        ...
        return (
            <form onSubmit={this.handleImageUpload}>
                ...
                <div className="form-group">
                    <input type="file" className="form-control" onChange={this.saveSelectedFiles} multiple/>
                </div>
                {!this.state.isLoading && 
                    <button type="submit" className="btn btn-success btn-block">Upload</button>}
                {this.state.isLoading && 
                    <button disabled={true} className="btn btn-success btn-block">Loading...</button>}
            </form >
        );
    }
}